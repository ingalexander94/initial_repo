...
class OperationModelModel {
  ...
  static findByPage = async (
    ...
    try {
      db = await connectCommonDB();
      let where = { om_id_model: id_model };
      if (parseInt(id_component) !== 0) {
        where = { ...where, operation_component: `C-${id_component}` };
      }
  ...
  static getTotalOperations = async (id_model = 0, id_component = 0) => {
    let db;
    try {
      db = await connectCommonDB();
      let where = { om_id_model: id_model };
