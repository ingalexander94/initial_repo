...
const AddOperationModal = ({ closeModal, title, operation }: Props) => {
        ...
        if (res) {
          ...
          if (operation) {
            const res2 = await callEndpoint(
              OperationService.getDocumentsByIdTeam(
  ...
  const handleChangeTeam = async (event: ChangeEvent<HTMLSelectElement>) => {
    ...
    setOptions([]);
    const res = await callEndpoint(
      OperationService.getDocumentsByIdTeam(value)
    );
    if (res) {
      const { data } = res.data;
      setComponents(data);
          ...
          <div className={styles.formGroup}>
            ...
            <div className={styles.formColumn}>
              ...
              <select
                ...
                value={formik.values.operation_component}
                onBlur={formik.handleBlur}
                onChange={formik.handleChange}
                className={`${styles.select} ${
                  formik.touched.operation_component &&
                ...
                {components.length &&
                  components.map(
                    (
                      ...
                      <option
                        key={index}
                        value={`${type_component}-${id_component}`}
                      >
